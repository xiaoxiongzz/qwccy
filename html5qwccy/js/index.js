...
console.log(userId, "window.location");
init();
...
function play() {
  ...
  startTimer(); // 确保在这里调用 startTimer
  peep();
  peep();
  peep();
  peep();
}
function updateUserTokenBalance(userId, score) {
  fetch(`https://tgserver.welikescwl.top/qwccy/users/${userId}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ token_balance: score })
  })
    .then(response => response.json())
    .then(data => {
      console.log('token_balance更新成功:', data);
    })
    .catch(error => {
      console.error('更新token_balance时出错:', error);
    });
}

// 更新用户highest_score
function updateUserHighestScore(userId, highestScore) {
  fetch(`https://tgserver.welikescwl.top/qwccy/users/${userId}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    ...
    .then(response => response.json())
    .then(data => {
      console.log('highest_score更新成功:', data);
    })
    .catch(error => {
      console.error('更新highest_score时出错:', error);
    });
}
function win() {
  playing = false;
  ...
  if (state.score > prevBest) {
    best = state.score;
    if (userId) {
      console.log(userId, best, "最高分");
      updateUserHighestScore(userId, best);
    }
  ...
  if (userId) {
    console.log(userId, state.score, "得分");
    updateUserTokenBalance(userId, state.score);
